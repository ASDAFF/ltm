...
if($type == 'yes'){
	if(CModule::IncludeModule("iblock") && CModule::IncludeModule("form")){
		//Все пользователи группы "Участники Киев подтвержденные (не все)"
		$userExelAr = array();
		$exExelAr = array();
		$exelAr = array();
		$filter = Array("GROUPS_ID"   => LuxorConfig::GROUP_USER_KIEV_P); 
		$i = 0;
		$rsUsers = CUser::GetList(($by="id"), ($order="desc"), $filter, array("SELECT"=>array("UF_ID_COMP", "UF_PAS", 'UF_ID3', 'UF_ID8')));
		while ($arUser = $rsUsers->Fetch()){
			...
			if($arUser['UF_ID3'] != ''){
				LuxorConfig::getAnswerFormSimple(
					LuxorConfig::ID_E_KIEV, 
					$arrAnswersVarnameE, 
					array('RESULT_ID'=>$arUser['UF_ID3'])
				);
				$keys1[] = $i;
			}	
		...
		foreach($arrAnswersVarnameE as $ex){
			if(trim($ex['SIMPLE_QUESTION_286'][0]['ANSWER_TEXT']) == 'Mr.' || trim($ex['SIMPLE_QUESTION_286'][0]['ANSWER_TEXT']) == 'None'){
				$hall = '';
			}else{
				$hall = $ex['SIMPLE_QUESTION_286'][0]['ANSWER_TEXT'];
			}
			$exExelAr[0][$keys1[$j]]['F_NAME']        = $ex['SIMPLE_QUESTION_896'][0]['USER_TEXT'];
			$exExelAr[0][$keys1[$j]]['L_NAME']        = $ex['SIMPLE_QUESTION_409'][0]['USER_TEXT'];
			$exExelAr[0][$keys1[$j]]['SOLUTION']      = $ex['SIMPLE_QUESTION_120'][0]['ANSWER_TEXT'];
			$exExelAr[0][$keys1[$j]]['JOB']           = $ex['SIMPLE_QUESTION_468'][0]['USER_TEXT'];
			$exExelAr[0][$keys1[$j]]['PHONE']         = $ex['SIMPLE_QUESTION_992'][0]['USER_TEXT'];
			$exExelAr[0][$keys1[$j]]['MAIL']          = $ex['SIMPLE_QUESTION_279'][0]['USER_TEXT'];
			$exExelAr[0][$keys1[$j]]['ALT_MAIL']      = $ex['SIMPLE_QUESTION_527'][0]['USER_TEXT'];
			$exExelAr[0][$keys1[$j]]['HALL']          = $hall;
			$exExelAr[0][$keys1[$j]]['TABLE']         = $ex['SIMPLE_QUESTION_471'][0]['USER_TEXT'];
			$j++;
		}
		$j = 0;
		foreach($arrAnswersVarnameE6 as $ex){
			...
			$exExelAr[5][$keys6[$j]]['F_NAME2']        = $ex['SIMPLE_QUESTION_896'][0]['USER_TEXT'];
			$exExelAr[5][$keys6[$j]]['L_NAME2']        = $ex['SIMPLE_QUESTION_409'][0]['USER_TEXT'];
			$exExelAr[5][$keys6[$j]]['SOLUTION2']      = $ex['SIMPLE_QUESTION_120'][0]['ANSWER_TEXT'];
			$exExelAr[5][$keys6[$j]]['JOB2']           = $ex['SIMPLE_QUESTION_468'][0]['USER_TEXT'];
			$exExelAr[5][$keys6[$j]]['PHONE2']         = $ex['SIMPLE_QUESTION_992'][0]['USER_TEXT'];
			$exExelAr[5][$keys6[$j]]['MAIL2']          = $ex['SIMPLE_QUESTION_279'][0]['USER_TEXT'];
			$exExelAr[5][$keys6[$j]]['ALT_MAIL2']      = $ex['SIMPLE_QUESTION_527'][0]['USER_TEXT'];
			$exExelAr[5][$keys6[$j]]['HALL2']          = $hall;
			$exExelAr[5][$keys6[$j]]['TABLE2']         = $ex['SIMPLE_QUESTION_471'][0]['USER_TEXT'];
			$j++;
		}
		$i = 0;
	...
	header('Content-Disposition: attachment;filename="Участники Киев подтвержденные.xls"');
	...
	if(CModule::IncludeModule("iblock") && CModule::IncludeModule("form")){
		//Все пользователи группы "Участники Киев подтвержденные (все)"
		$userExelAr = array();
		$exExelAr = array();
		$exelAr = array();
		$filter = Array("GROUPS_ID"   => LuxorConfig::GROUP_USER_KIEV_P); 
	...
	// Redirect output to a client’s web browser (Excel5)
	header('Content-Type: application/vnd.ms-excel');
	header('Content-Disposition: attachment;filename="Участники Киев подтвержденные (коллеги отдельной строкой).xls"');
	header('Cache-Control: max-age=0');
	// If you're serving to IE 9, then the following may be needed
	header('Cache-Control: max-age=1');
	...
	if(CModule::IncludeModule("iblock") && CModule::IncludeModule("form")){
		//Все пользователи группы "Участники Киев неподтвержденные (не все)"
		$userExelAr = array();
		$exExelAr = array();
		$exelAr = array();
		$filter = Array("GROUPS_ID"   => LuxorConfig::GROUP_USER_KIEV_NP); 
		$i = 0;
		$rsUsers = CUser::GetList(($by="id"), ($order="desc"), $filter, array("SELECT"=>array("UF_ID_COMP", "UF_PAS", 'UF_ID3', 'UF_ID8')));
		while ($arUser = $rsUsers->Fetch()){
			$userExelAr[$i]['ID']       = $arUser['ID'];
	...
	// Redirect output to a client’s web browser (Excel5)
	header('Content-Type: application/vnd.ms-excel');
	header('Content-Disposition: attachment;filename="Участники Киев неподтвержденные (не все).xls"');
	header('Cache-Control: max-age=0');
	// If you're serving to IE 9, then the following may be needed
	header('Cache-Control: max-age=1');
	...
	if(CModule::IncludeModule("iblock") && CModule::IncludeModule("form")){
		//Все пользователи группы "Участники Киев неподтвержденные (все)"
		$userExelAr = array();
		$exExelAr = array();
		$exelAr = array();
		$filter = Array("GROUPS_ID"   => LuxorConfig::GROUP_USER_KIEV_NP); 
	...
	// Redirect output to a client’s web browser (Excel5)
	header('Content-Type: application/vnd.ms-excel');
	header('Content-Disposition: attachment;filename="Участники Киев неподтвержденные (все).xls"');
	header('Cache-Control: max-age=0');
	// If you're serving to IE 9, then the following may be needed
	header('Cache-Control: max-age=1');
