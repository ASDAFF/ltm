...
if($type == 'yes'){
	if(CModule::IncludeModule("iblock") && CModule::IncludeModule("form")){
		//Все пользователи группы "Участники Москва Весна 2015 подтвержденные (не все)"
		$userExelAr = array();
		$exExelAr = array();
		$exelAr = array();
		$filter = Array("GROUPS_ID"   => LuxorConfig::GROUP_USER_MS15_P); 
		$i = 0;
		$rsUsers = CUser::GetList(($by="id"), ($order="desc"), $filter, array("SELECT"=>array("UF_ID_COMP", "UF_PAS", 'UF_ID11', 'UF_ID12')));
		while ($arUser = $rsUsers->Fetch()){
			...
			if($arUser['UF_ID11'] != ''){
				LuxorConfig::getAnswerFormSimple(
					LuxorConfig::ID_E_MOSC_SP15, 
					$arrAnswersVarnameE, 
					array('RESULT_ID'=>$arUser['UF_ID11'])
				);
				$keys1[] = $i;
			}	
			...
			if($arUser['UF_ID12'] != ''){
				LuxorConfig::getAnswerFormSimple(
					LuxorConfig::ID_E_MOSC_SP15,  
					$arrAnswersVarnameE6, 
					array('RESULT_ID'=>$arUser['UF_ID12'])
				);
				$keys6[] = $i;
			}
		...
		foreach($arrAnswersVarnameE as $ex){
			if(trim($ex['SIMPLE_QUESTION_713'][0]['ANSWER_TEXT']) == 'Mr.' || trim($ex['SIMPLE_QUESTION_713'][0]['ANSWER_TEXT']) == 'None'){
				$hall = '';
			}else{
				$hall = $ex['SIMPLE_QUESTION_713'][0]['ANSWER_TEXT'];
			}
			...
			$exExelAr[0][$keys1[$j]]['TABLE']         = $ex['SIMPLE_QUESTION_418'][0]['USER_TEXT'];
			$j++;
		}
		$j = 0;
		foreach($arrAnswersVarnameE6 as $ex){
			...
			$exExelAr[5][$keys6[$j]]['F_NAME2']        = $ex['SIMPLE_QUESTION_446'][0]['USER_TEXT'];
			$exExelAr[5][$keys6[$j]]['L_NAME2']        = $ex['SIMPLE_QUESTION_551'][0]['USER_TEXT'];
			$exExelAr[5][$keys6[$j]]['SOLUTION2']      = $ex['SIMPLE_QUESTION_889'][0]['ANSWER_TEXT'];
			$exExelAr[5][$keys6[$j]]['JOB2']           = $ex['SIMPLE_QUESTION_729'][0]['USER_TEXT'];
			$exExelAr[5][$keys6[$j]]['PHONE2']         = $ex['SIMPLE_QUESTION_394'][0]['USER_TEXT'];
			$exExelAr[5][$keys6[$j]]['MAIL2']          = $ex['SIMPLE_QUESTION_859'][0]['USER_TEXT'];
			$exExelAr[5][$keys6[$j]]['ALT_MAIL2']      = $ex['SIMPLE_QUESTION_749'][0]['USER_TEXT'];
			$exExelAr[5][$keys6[$j]]['HALL2']          = $hall;
			$exExelAr[5][$keys6[$j]]['TABLE2']         = $ex['SIMPLE_QUESTION_418'][0]['USER_TEXT'];
			$j++;
		}
		$i = 0;
	...
	header('Content-Disposition: attachment;filename="Участники Москва Весна 2015 подтвержденные.xls"');
	...
	if(CModule::IncludeModule("iblock") && CModule::IncludeModule("form")){
		//Все пользователи группы "Участники Москва Весна 2015 подтвержденные (все)"
		$userExelAr = array();
		$exExelAr = array();
		$exelAr = array();
		$filter = Array("GROUPS_ID"   => LuxorConfig::GROUP_USER_MS15_P); 
	...
	// Redirect output to a client’s web browser (Excel5)
	header('Content-Type: application/vnd.ms-excel');
	header('Content-Disposition: attachment;filename="Участники Москва Весна 2015 подтвержденные (коллеги отдельно).xls"');
	header('Cache-Control: max-age=0');
	// If you're serving to IE 9, then the following may be needed
	header('Cache-Control: max-age=1');
	...
	if(CModule::IncludeModule("iblock") && CModule::IncludeModule("form")){
		//Все пользователи группы "Участники Москва Весна 2015 неподтвержденные (не все)"
		$userExelAr = array();
		$exExelAr = array();
		$exelAr = array();
		$filter = Array("GROUPS_ID"   => LuxorConfig::GROUP_USER_MS15_NP); 
		$i = 0;
		$rsUsers = CUser::GetList(($by="id"), ($order="desc"), $filter, array("SELECT"=>array("UF_ID_COMP", "UF_PAS", 'UF_ID11', 'UF_ID12')));
		while ($arUser = $rsUsers->Fetch()){
			$userExelAr[$i]['ID']       = $arUser['ID'];
	...
	// Redirect output to a client’s web browser (Excel5)
	header('Content-Type: application/vnd.ms-excel');
	header('Content-Disposition: attachment;filename="Участники Москва Весна 2015 неподтвержденные (не все).xls"');
	header('Cache-Control: max-age=0');
	// If you're serving to IE 9, then the following may be needed
	header('Cache-Control: max-age=1');
	...
	if(CModule::IncludeModule("iblock") && CModule::IncludeModule("form")){
		//Все пользователи группы "Участники Москва Весна 2015 неподтвержденные (все)"
		$userExelAr = array();
		$exExelAr = array();
		$exelAr = array();
		$filter = Array("GROUPS_ID"   => LuxorConfig::GROUP_USER_MS15_NP); 
	...
	// Redirect output to a client’s web browser (Excel5)
	header('Content-Type: application/vnd.ms-excel');
	header('Content-Disposition: attachment;filename="Участники Москва Весна 2015 неподтвержденные (все).xls"');
	header('Cache-Control: max-age=0');
	// If you're serving to IE 9, then the following may be needed
	header('Cache-Control: max-age=1');
