...
if($type == 'guests'){
	if(CModule::IncludeModule("iblock") && CModule::IncludeModule("form")){
		//Все пользователи группы "Гости Киев подтвержденные" на утро
		$userExelAr = array();
		$exExelAr = array();
		$exelAr = array();
		$filter = Array("GROUPS_ID"   => LuxorConfig::GROUP_GUEST_K, 'UF_MR'=>'1'); 
		$i = 0;
	...
	header('Content-Disposition: attachment;filename="Гости Киев на утро.xls"');
	...
	if(CModule::IncludeModule("iblock") && CModule::IncludeModule("form")){
		//Все пользователи группы "Гости Киев подтвержденные" на утро (все)
		$userExelAr = array();
		$exExelAr = array();
		$exelAr = array();
		$filter = Array("GROUPS_ID"   => LuxorConfig::GROUP_GUEST_K, 'UF_MR'=>'1'); 
	...
	// Redirect output to a client’s web browser (Excel5)
	header('Content-Type: application/vnd.ms-excel');
	header('Content-Disposition: attachment;filename="Гости Киев на утро (все люди).xls"');
	header('Cache-Control: max-age=0');
	// If you're serving to IE 9, then the following may be needed
	header('Cache-Control: max-age=1');
	...
	if(CModule::IncludeModule("iblock") && CModule::IncludeModule("form")){
		//Все пользователи группы "Гости Киев подтвержденные" hosted buyers
		$userExelAr = array();
		$exExelAr = array();
		$exelAr = array();
		$filter = Array("GROUPS_ID"   => LuxorConfig::GROUP_GUEST_K, 'UF_HB'=>'1'); 
		$i = 0;
	...
	// Redirect output to a client’s web browser (Excel5)
	header('Content-Type: application/vnd.ms-excel');
	header('Content-Disposition: attachment;filename="Гости Киев (Hosted Buyers).xls"');
	header('Cache-Control: max-age=0');
	// If you're serving to IE 9, then the following may be needed
	header('Cache-Control: max-age=1');
	...
	if(CModule::IncludeModule("iblock") && CModule::IncludeModule("form")){
		...
		$exExelAr = array();
		$exelAr = array();
		$filter = Array("GROUPS_ID"   => LuxorConfig::GROUP_GUEST_K, 'UF_EV'=>'1'); 
	...
	// Redirect output to a client’s web browser (Excel5)
	header('Content-Type: application/vnd.ms-excel');
	header('Content-Disposition: attachment;filename="Гости Киев вечер.xls"');
	header('Cache-Control: max-age=0');
	// If you're serving to IE 9, then the following may be needed
	header('Cache-Control: max-age=1');
	...
	// Redirect output to a client’s web browser (Excel5)
	header('Content-Type: application/vnd.ms-excel');
	header('Content-Disposition: attachment;filename="Гости Киев вечер (все люди).xls"');
	header('Cache-Control: max-age=0');
	// If you're serving to IE 9, then the following may be needed
	header('Cache-Control: max-age=1');
	...
	if(CModule::IncludeModule("iblock") && CModule::IncludeModule("form")){
		//Все пользователи группы "Гости Киев неподтвержденные"
		$userExelAr = array();
		$exExelAr = array();
		$exelAr = array();
		$filter = Array("GROUPS_ID"   => LuxorConfig::GROUP_GUEST_INACT, '!UF_ID3'=>'false'); 
		$rsUsers = CUser::GetList(($by="id"), ($order="desc"), $filter, array("SELECT"=>array("UF_ID_COMP", "UF_PAS", 'UF_ID2', 'UF_ID3', 'UF_EV')));
		$i = 0;
		while ($arUser = $rsUsers->Fetch()){
			$exelAr[$i]['ID']       = $arUser['ID'];
	...
	// Redirect output to a client’s web browser (Excel5)
	header('Content-Type: application/vnd.ms-excel');
	header('Content-Disposition: attachment;filename="Гости Киев неподтвержденные.xls"');
	header('Cache-Control: max-age=0');
	// If you're serving to IE 9, then the following may be needed
	header('Cache-Control: max-age=1');
	...
	if(CModule::IncludeModule("iblock") && CModule::IncludeModule("form")){
		//Все пользователи группы "Гости Киев неподтвержденные" все
		$userExelAr = array();
		$exExelAr = array();
		$exelAr = array();
		$filter = Array("GROUPS_ID"   => LuxorConfig::GROUP_GUEST_K, '!UF_ID3'=>'false'); 
		$rsUsers = CUser::GetList(($by="id"), ($order="desc"), $filter, array("SELECT"=>array("UF_ID_COMP", "UF_PAS", 'UF_ID2', 'UF_ID3', 'UF_EV')));
		$i = 0;
		while ($arUser = $rsUsers->Fetch()){
			$exelAr[$i]['ID']       = $arUser['ID'];
	...
	// Redirect output to a client’s web browser (Excel5)
	header('Content-Type: application/vnd.ms-excel');
	header('Content-Disposition: attachment;filename="Гости Киев неподтвержденные(все люди).xls"');
	header('Cache-Control: max-age=0');
	// If you're serving to IE 9, then the following may be needed
	header('Cache-Control: max-age=1');
