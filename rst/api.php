...
$arSelect = Array("ID", "NAME", "PROPERTY_USER_GROUP_ID", "PROPERTY_UC_PARTICIPANTS_GROUP", "PROPERTY_C_GUESTS_GROUP", "PROPERTY_UC_GUESTS_GROUP", "PROPERTY_DATE_TIME", "PROPERTY_APP_ID");
...
while($ob = $res->GetNextElement()) {
    ...
    define('EXHIBITION_ID',$arFields["PROPERTY_APP_ID_VALUE"]);//APP_ID
}
                ...
                while($arUsers = $rsUsers->getNext()) {
                    ...
                    foreach ($arGroups as $grp) {
                        ...
                        if ($grp==54) {
                            ...
                            $result["PERSONAL_PHOTO"] = "http://".SITE_URL_PATH."/image.php?src=/upload/main/396/unnamed.jpg&w=242&h=242&zc=4";
...
<p>This B2B event has been carefully thought out and planned in such a way that sellers and potential buyers of services – companies working with VIP clients, have the opportunity to establish partnerships within the space of a single day. The main aim of the of the organisers of the Luxury Travel Mart is to expand the geography of sales of the exhibition’s participants and also to introduce players to new areas and services to which they previously did not have access.</p>";
                    ...
                    $sql = mysqli_query($this->db,"SELECT * FROM `chat_messages` WHERE (`AUTHOR_ID` = '" . $arUsers["ID"] . "' AND `TO_USER` = '" . $USER->GetID() . "') OR (`AUTHOR_ID` = '" . $USER->GetID() . "' AND `TO_USER` = '" . $arUsers["ID"] . "') ORDER BY ID DESC" );
                    $result["MESSAGES_COUNT"] = intval(mysqli_num_rows($sql));
...
*/
            ...
            foreach ($arUserMe["UF_FAVORITES"] as $fav) {
                ...
                $sql = mysqli_query($this->db,"SELECT * FROM `chat_messages` WHERE (`AUTHOR_ID` = '" . $arUser["ID"] . "' AND `TO_USER` = '" . $USER->GetID() . "') OR (`AUTHOR_ID` = '" . $USER->GetID() . "' AND `TO_USER` = '" . $arUser["ID"] . "')  ORDER BY ID DESC");
                $result[$k]["MESSAGES_COUNT"] = intval(mysqli_num_rows($sql));
            ...
            while($arUser = $rsUsers->getNext()) {
                ...
                $result["ADMIN"]["PERSONAL_PHOTO"] = "http://".SITE_URL_PATH."/image.php?src=/upload/main/396/unnamed.jpg&w=242&h=242&zc=4";
...
<p>This B2B event has been carefully thought out and planned in such a way that sellers and potential buyers of services – companies working with VIP clients, have the opportunity to establish partnerships within the space of a single day. The main aim of the of the organisers of the Luxury Travel Mart is to expand the geography of sales of the exhibition’s participants and also to introduce players to new areas and services to which they previously did not have access.</p>";
            ...
            $sql = mysqli_query($this->db,"SELECT * FROM `chat_admin_messages` WHERE `AUTHOR_ID` = '" . $USER->GetID() . "' OR `TO_USER` = '" . $USER->GetID() . "' GROUP BY TO_USER  ORDER BY ID DESC");
            $adminmessagesCount = intval(mysqli_num_rows($sql));
            ...
            while ($arUsers = $rsUsers->getNext()) {
                ...
                $sql = mysqli_query($this->db,"SELECT * FROM `chat_messages` WHERE (`AUTHOR_ID` = '" . $arUser["ID"] . "' AND `TO_USER` = '" . $USER->GetID() . "') OR (`AUTHOR_ID` = '" . $USER->GetID() . "' AND `TO_USER` = '" . $arUser["ID"] . "')  ORDER BY ID DESC");
                $result["USERS"][$k]["MESSAGES_COUNT"] = intval(mysqli_num_rows($sql));
            ...
            if (intval($this->_request['user_id'])>0) {
                ...
                $sql = mysqli_query($this->db,"SELECT * FROM `chat_messages` WHERE `AUTHOR_ID` = '" . $this->_request['user_id'] . "' OR `TO_USER` = '" . $this->_request['user_id'] . "' GROUP BY TO_USER  ORDER BY ID DESC");
                $messagesCount = intval(mysqli_num_rows($sql));

                $sql = mysqli_query($this->db,"SELECT * FROM `chat_admin_messages` WHERE `AUTHOR_ID` = '" . $this->_request['user_id'] . "' OR `TO_USER` = '" . $this->_request['user_id'] . "' GROUP BY TO_USER  ORDER BY ID DESC");
                $adminmessagesCount = intval(mysqli_num_rows($sql));
...
*/
                ...
                mysqli_query($this->db,"INSERT INTO `b_vote_event` (`ID`, `VOTE_ID`, `VOTE_USER_ID`, `DATE_VOTE`, `STAT_SESSION_ID`, `IP`, `VALID`) VALUES (NULL, '".$VOTE_ID."', '".$USER->GetId()."', '".date("Y-m-d H:i:s",time())."', '', '".$_SERVER["REMOTE_ADDR"]."', 'Y')");
                $eventID = mysqli_insert_id();

                if ($this->_request["points"]) {
                    mysqli_query($this->db,"INSERT INTO `b_vote_event_question` (`ID`, `EVENT_ID`, `QUESTION_ID`) VALUES (NULL, '" . $eventID . "', '13')");
                    $eventQID = mysqli_insert_id();
                    mysqli_query($this->db,"INSERT INTO `b_vote_event_answer` (`ID`, `EVENT_QUESTION_ID`, `ANSWER_ID`, `MESSAGE`) VALUES (NULL, '".$eventQID."', '41', '".$this->_request["points"]."')");
                }
                if ($this->_request["buyer_id"]) {
                    mysqli_query($this->db,"INSERT INTO `b_vote_event_question` (`ID`, `EVENT_ID`, `QUESTION_ID`) VALUES (NULL, '" . $eventID . "', '14')");
                    $eventQID = mysqli_insert_id();
                    mysqli_query($this->db,"INSERT INTO `b_vote_event_answer` (`ID`, `EVENT_QUESTION_ID`, `ANSWER_ID`, `MESSAGE`) VALUES (NULL, '".$eventQID."', '42', '".$this->_request["buyer_id"]."')");
                }
                ...
                foreach ($this->_request["answ"] as $k=>$v) {
                    foreach ($v as $k2=>$v2) {
                        ...
                        if (!$k2 OR $k2==0) {
                            if ($k AND $v2) {
                                mysqli_query($this->db,"INSERT INTO `b_vote_event_question` (`ID`, `EVENT_ID`, `QUESTION_ID`) VALUES (NULL, '" . $eventID . "', '" . $k . "')");
                                $eventQID = mysqli_insert_id();
                                mysqli_query($this->db,"INSERT INTO `b_vote_event_answer` (`ID`, `EVENT_QUESTION_ID`, `ANSWER_ID`, `MESSAGE`) VALUES (NULL, '" . $eventQID . "', '" . $v2 . "', '')");
                            }
                        } else {
                            if ($k2 AND $v2) {
                                mysqli_query($this->db,"INSERT INTO `b_vote_event_question` (`ID`, `EVENT_ID`, `QUESTION_ID`) VALUES (NULL, '" . $eventID . "', '" . $k2 . "')");
                                $eventQID = mysqli_insert_id();
                                mysqli_query($this->db,"INSERT INTO `b_vote_event_answer` (`ID`, `EVENT_QUESTION_ID`, `ANSWER_ID`, `MESSAGE`) VALUES (NULL, '" . $eventQID . "', '" . $v2 . "', '')");
                            }
                        }
            ...
            if (intval($this->_request['user_id'])>0) {
                ...
                $result = mysqli_query($this->db,"INSERT INTO  `chat_messages` (`ID`,`AUTHOR_ID`,`TO_USER`,`MESSAGE`,`DATE_CREATE`) VALUES (NULL,'".$USER->GetId()."','".$this->_request["user_id"]."','".strip_tags($USER->GetFullName().' added you to favorites list')."','".$data."')");
            ...
            if (intval($this->_request['user_id'])>0) {
                ...
                $data = date("Y-m-d H:i:s", time());
                $result = mysqli_query($this->db,"INSERT INTO  `chat_messages` (`ID`,`AUTHOR_ID`,`TO_USER`,`MESSAGE`,`DATE_CREATE`) VALUES (NULL,'".$USER->GetId()."','".$this->_request["user_id"]."','".strip_tags($USER->GetFullName().' invites you for the next appointment')."','".$data."')");
                $ID = mysqli_insert_id($this->db);
            ...
            if (intval($this->_request['user_id'])>0) {
                ...
                $sql = mysqli_query($this->db,"SELECT * FROM `meetings_requests` WHERE `EXHIBITION_ID` ='".EXHIBITION_ID."' AND `SENDER_ID` = '".$USER->GetId()."' AND `RECEIVER_ID`='".$this->_request['user_id']."' ORDER BY TIMESLOT_ID LIMIT 0,1");
                if(intval(mysqli_num_rows($sql)) > 0) {
                ...
                $data = date("Y-m-d H:i:s", time());
                $result = mysqli_query($this->db,"INSERT INTO  `no_show_reports` (`ID`,`MEETING_ID`,`SENDER_ID`,`RECEIVER_ID`,`REPORT_TIME`) VALUES (NULL,'".$meeting_id."','".$USER->GetId()."','".$this->_request["user_id"]."','".$data."')");
                $ID = mysqli_insert_id($this->db);
...
//OR `RECEIVER_ID` = '".$this->_request["SENDER_ID"]."'
                $k=0;
                $sql = mysqli_query($this->db,"SELECT * FROM `meetings_requests` WHERE `STATUS` = 'confirmed' AND `EXHIBITION_ID` ='".EXHIBITION_ID."' AND (`SENDER_ID` = '".$this->_request["SENDER_ID"]."' OR `RECEIVER_ID` = '".$this->_request["SENDER_ID"]."') ORDER BY TIMESLOT_ID" );
                ...
                if(intval(mysqli_num_rows($sql)) > 0) {
                    while ($row = mysqli_fetch_array($sql, MYSQLI_ASSOC)) {
                        ...
                        if ($is_fav) $result[$k]["RECEIVER"]["IS_FAVORITE"] = 1; else $result[$k]["RECEIVER"]["IS_FAVORITE"] = 0;
                            ...
                            $sql2 = mysqli_query($this->db,"SELECT * FROM `meetings_timeslots` WHERE `ID` = '".$row["TIMESLOT_ID"]."' AND `EXHIBITION_ID` = '".EXHIBITION_ID."'  " );
    ...
    private function chat(){
        ...
        if ($this->_request['func']=="getMessage" AND $this->get_request_method() == "GET") {
            if (intval($this->_request["ID"]) > 0 AND intval($this->_request["AUTHOR_ID"]) > 0 AND intval($this->_request["TO_USER"]) > 0) {
                ...
                $sql = mysqli_query($this->db,"SELECT * FROM `chat_messages` WHERE `ID` = '" . $this->_request["ID"] . "' AND `AUTHOR_ID` = '" . $this->_request["AUTHOR_ID"] . "' AND TO_USER = '" . $this->_request["TO_USER"] . "' ORDER BY ID DESC");
                if (intval(mysqli_num_rows($sql)) > 0) {
                    while ($row = mysqli_fetch_array($sql, MYSQLI_ASSOC)) {
            ...
            if (intval($this->_request["ID"])>0 AND intval($this->_request["AUTHOR_ID"])>0 AND intval($this->_request["TO_USER"])>0) {
                ...
                $sql = mysqli_query($this->db,"SELECT * FROM `chat_admin_messages` WHERE `ID` = '".$this->_request["ID"]."' AND `AUTHOR_ID` = '".$this->_request["AUTHOR_ID"]."' AND TO_USER = '".$this->_request["TO_USER"]."' ORDER BY ID DESC");
                if(intval(mysqli_num_rows($sql)) > 0) {
                    while ($row = mysqli_fetch_array($sql, MYSQLI_ASSOC)) {
                        $result["ID"] = iconv("CP1251","UTF-8",$row["ID"]);
                        $result["MESSAGE"] = iconv("CP1251","UTF-8",$row["MESSAGE"]);
            ...
            if (intval($this->_request["AUTHOR_ID"])>0 AND intval($this->_request["TO_USER"])>0) {
                ...
                $sql = mysqli_query($this->db,"SELECT * FROM `chat_messages` WHERE (AUTHOR_ID = '".$this->_request["AUTHOR_ID"]."' AND TO_USER = '".$this->_request["TO_USER"]."') OR ( TO_USER = '".$this->_request["AUTHOR_ID"]."' AND AUTHOR_ID = '".$this->_request["TO_USER"]."') ORDER BY ID DESC LIMIT ".$offset.",".$limit);
                if(intval(mysqli_num_rows($sql)) > 0) {
                    $k=0;
                    while ($row = mysqli_fetch_array($sql, MYSQLI_ASSOC)) {
                        $result[$k]["ID"] = $row["ID"];
            ...
            if (intval($this->_request["TO_USER"])>0) {
                ...
                $sql = mysqli_query($this->db,"SELECT * FROM `chat_admin_messages` WHERE (AUTHOR_ID = '".$admin_id."' AND TO_USER = '".$this->_request["TO_USER"]."') OR ( TO_USER = '".$admin_id."' AND AUTHOR_ID = '".$this->_request["TO_USER"]."') ORDER BY ID DESC LIMIT ".$offset.",".$limit);
                if(intval(mysqli_num_rows($sql)) > 0) {
                    $k=0;
                    while ($row = mysqli_fetch_array($sql, MYSQLI_ASSOC)) {
            ...
            $sql = mysqli_query($this->db,"SELECT * FROM `chat_messages` WHERE (AUTHOR_ID = '" . $USER->GetId() . "' OR TO_USER = '" . $USER->GetId() . "')  GROUP BY TO_USER ORDER BY ID DESC ");
            if (intval(mysqli_num_rows($sql)) > 0)  {
                $k=0;
                while ($row = mysqli_fetch_array($sql, MYSQLI_ASSOC)) {
                    ...
                    if (!$this_admin AND $arUser["ID"]!=$USER->GetID()) {
                        ...
                        $last_date = $last_mess = '';
                        $sql2 = mysqli_query($this->db,"SELECT * FROM `chat_messages` WHERE (AUTHOR_ID = '" . $USER->GetId() . "' AND TO_USER = '" . $row["TO_USER"] . "') OR (AUTHOR_ID = '" . $row["TO_USER"] . "' AND TO_USER = '" . $USER->GetId() . "') ORDER BY ID DESC LIMIT 0,1 ");
                        while ($row2 = mysqli_fetch_array($sql2, MYSQLI_ASSOC)) {
            ...
            if (intval($this->_request["AUTHOR_ID"])>0 AND $this->_request["MESSAGE"] AND intval($this->_request["TO_USER"])>0) {
                $data = date("Y-m-d H:i:s", time());
                $result = mysqli_query($this->db,"INSERT INTO  `chat_messages` (`ID`,`AUTHOR_ID`,`TO_USER`,`MESSAGE`,`DATE_CREATE`) VALUES (NULL,'".$this->_request["AUTHOR_ID"]."','".$this->_request["TO_USER"]."','".strip_tags($this->_request["MESSAGE"])."','".$data."')");
                $ID = mysqli_insert_id($this->db);
            ...
            if (intval($this->_request["AUTHOR_ID"])>0 AND $this->_request["MESSAGE"] AND intval($this->_request["TO_USER"])>0) {
                $data = date("Y-m-d H:i:s", time());
                $result = mysqli_query($this->db,"INSERT INTO  `chat_admin_messages` (`ID`,`AUTHOR_ID`,`TO_USER`,`MESSAGE`,`DATE_CREATE`) VALUES (NULL,'".$this->_request["AUTHOR_ID"]."','".$this->_request["TO_USER"]."','".strip_tags($this->_request["MESSAGE"])."','".$data."') ");
                $ID = mysqli_insert_id($this->db);
