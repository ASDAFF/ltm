...
// ************************************************************* //
						...
						if($arParams["REGIST_USER"] == "Y" && $arParams["REGIST_GROUP"] != '' && $arParams["LOGIN_FIELD"] != ''){
							...
							$newLogin = trim($_POST[$arParams["LOGIN_FIELD"]]);
							$newEmail = trim($_POST[$arParams["EMAIL_FIELD"]]);
							$password = $_POST[$arParams["PASSWORD_FIELD"]];
							$newPassword = $password;
							
							if(in_array(166,$_POST["form_checkbox_visit_format"])){
								$user = new CUser;
								$arFields = Array(
								  "EMAIL"             => $newEmail,
								  "LOGIN"             => $newLogin,
								  ...
								  "UF_ANKETA"         => $RESULT_ID,
								  "NAME"	          => $_POST["form_text_128"],
								  "LAST_NAME"         => $_POST["form_text_129"],
								  "WORK_COMPANY"      => $_POST["form_text_130"],
								);
								...
								if($ID){
									$arEventFields = array(
										...
										"PASSWORD"          => $password,
										"UF_ANKETA"         => $RESULT_ID,
										"COMPANY"	          => $_POST["form_text_130"]
										);
									CEvent::Send("NEW_GUEST", "s1", $arEventFields);
								}
								else{
									$arEventFields = array(
									...
									CEvent::Send("NEW_GUEST_ERROR", "s1", $arEventFields);
									}
								if(in_array(162,$_POST["form_checkbox_visit_format"])){
									CEvent::Send("NEW_EVENING_GUEST", "s1", $arEventFields);
								}
							}
							else{
								$newPassword = "EvGuestPass123";
								$newLogin = "evening".$RESULT_ID.$newEmail;
								$user = new CUser;
								$arFields = Array(
								  "EMAIL"             => $newEmail,
								  "LOGIN"             => $newLogin,
								...
								if($ID){
									...
									CEvent::Send("NEW_EVENING_GUEST_ERROR", "s1", $arEventFields);
									}
							}
						}	
