...
/*/////////// ÏÐÎÂÅÐÊÀ ÑÎÂÏÀÄÅÍÈß ÏÎËß EMAIL ///////////*/
			if(!isset($arParams["EMAIL_PREFIX"])){
				$arParams["EMAIL_PREFIX"] = '';
			}

			if(isset($_REQUEST[$_POST[$arParams["EMAIL_FIELD"]]]) && $_REQUEST[$_POST[$arParams["EMAIL_FIELD"]]] != $_REQUEST["form_email_conf"]){
			...
			if($arParams["REGIST_USER"] == 'Y' && $arParams["EMAIL_FIELD"]){
				$tempUser = CUser::GetByLogin(trim($arParams["EMAIL_PREFIX"].$_REQUEST[$arParams["EMAIL_FIELD"]]));
...
// ************************************************************* //
						if($arParams["REGIST_USER"] == "Y" && $arParams["REGIST_GROUP"] != '' && $arParams["EMAIL_FIELD"] != ''){
							...
							$prefix = $arParams["EMAIL_PREFIX"];

							$user = new CUser;
							$arFields = Array(
							  "EMAIL"             => $newEmail,
							  "LOGIN"             => $prefix.$newEmail,
							  ...
							  "LAST_NAME"         => $_POST["form_text_2"],
							  "WORK_COMPANY"      => $_POST["form_text_10"],
							);
