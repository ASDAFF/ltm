...
if(!function_exists("GetUserName"))
{
	...
	function GetParticipantCompanyID($USER_ID)
	{
		$ar_res = false;
...
if($USER->IsAdmin()){
	$arResult["count"] = CForumPrivateMessage::PMSize(1, COption::GetOptionInt("forum", "MaxPrivateMessages", 100));
}
else{
...
if (!empty($arResult["action"]))
{
	...
	if($USER->IsAdmin()){
		BXClearCache(true, "/bitrix/forum/user/".intVal(1)."/");
	...
	foreach ($arComponentPath as $path)
	{
		...
		if($USER->IsAdmin()){
			$path = str_replace("//", "/", $componentRelativePath."/user1");
		}
		else{
...
$arResult["SortingEx"]["COMPANY"] = SortingEx("company");
...
if($dbrMessages && $arMsg = $dbrMessages->GetNext())
{
	do
	{
		...
		$arMsg["SHOW_COMPANY"] = GetUserCompany($arMsg[$arResult["InputOutput"]]);
		$arMsg["SHOW_COMPANY_ID"] = GetParticipantCompanyID($arMsg[$arResult["InputOutput"]]);
		$arMsg["URL"] = array(
...
/************** Folders ********************************************/
if($USER->IsAdmin()){
	$resFolder = CForumPMFolder::GetList(array(), array("USER_ID" => 1));
}
else{
