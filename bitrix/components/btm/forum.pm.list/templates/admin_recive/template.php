...
<?
	...
	$class .= ($res['IS_READ'] == 'N')?" new-message":"";
?>
        <tr class="<?= $class?>">
        	<td align="center"><? if($res["IS_READ"] == 'N'){?><img src="<?= SITE_TEMPLATE_PATH?>/images/envelope.gif" /><? }else{?>&nbsp;<? }?></td>
            <td>
            <?=$res["POST_SUBJ"]?><br />
            <strong><a href="/admin/service/read.php?mes=<?=$res["ID"]?>" target="_blank" onclick="newWind('/admin/service/read.php?mes=<?=$res["ID"]?>'); return false;">Open</a></strong>, <strong><a href="/admin/service/write.php?id=<?=$res["AUTHOR_ID"]?>&mes=<?=$res["ID"]?>" target="_blank" onclick="newWind('/admin/service/write.php?id=<?=$res["AUTHOR_ID"]?>&mes=<?=$res["ID"]?>'); return false;">Reply</a></strong>
            </td>
