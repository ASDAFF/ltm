...
if (CModule::IncludeModule("form"))
{
	...
	if(strLen($arParams["ANKETA_TO"])<=0){
		$arParams["ANKETA_TO"] = "UF_ANKETA";
	}
	
	if(strLen($arParams["ANKETA_FROM"])<=0){
		$arParams["ANKETA_TO"] = "UF_ANKETA_PREV";
	}
	...
	$arUser = $rsUser->Fetch();
	$arResult["ANKETA_PREV"] = $arUser["UF_ANKETA"];
...
/*/////////// ÏÐÎÂÅÐÊÀ ÑÎÂÏÀÄÅÍÈß ÏÎËß EMAIL ///////////*/
			...
			if($arParams["REGIST_USER"] == 'Y' && $arParams["EMAIL_FIELD"]){
				$tempUser = CUser::GetByLogin($_REQUEST[$arParams["EMAIL_FIELD"]]);
...
// ************************************************************* //
//                   ÐÅÄÀÊÒÈÐÓÅÌ ÏÎËÜÇÎÂÀÒÅËß                    //
// ************************************************************* //
						if($arParams["REGIST_USER"] == "Y" && $arParams["REGIST_GROUP"] != '' && $arParams["EMAIL_FIELD"] != ''){
							
								$user = new CUser;
								$fields = Array(
								  $arParams["ANKETA_TO"]         => $RESULT_ID,
								);
								$user->Update($USER->GetID(), $fields);
								$arGroups = CUser::GetUserGroup($USER->GetID());
								$arGroups[] = $arParams["REGIST_GROUP"];
								CUser::SetUserGroup($USER->GetID(), $arGroups);

								$strEdirErr = $user->LAST_ERROR;
								$newEmail = trim($_POST[$arParams["EMAIL_FIELD"]]);
							...
							if(!$strEdirErr){
								...
								CEvent::Send("NEW_PARTICIP", "s1", $arEventFields);
							}
							else{
								$arEventFields = array(
		...
		*/
		...
		CForm::GetResultAnswerArray("1", 
			$arrColumns, 
			$arrAnswers, 
			$arrAnswersVarname, 
			array("RESULT_ID" => $arResult["ANKETA_PREV"]));
		
		$arrAnswersVarname = $arrAnswersVarname[$arResult["ANKETA_PREV"]];

		foreach ($arResult["QUESTIONS"] as $key => $arQuestion){
			if($arResult["QUESTIONS"][$key]["STRUCTURE"][0]["FIELD_TYPE"] == text || $arResult["QUESTIONS"][$key]["STRUCTURE"][0]["FIELD_TYPE"] == email){
				$arResult["QUESTIONS"][$key]["HTML_CODE"] = str_replace('value=""', 'value="'.$arrAnswersVarname[$key][0]["USER_TEXT"].'"', $arResult["QUESTIONS"][$key]["HTML_CODE"]);
			}
			if($arResult["QUESTIONS"][$key]["STRUCTURE"][0]["FIELD_TYPE"] == dropdown && strpos($arResult["QUESTIONS"][$key]["HTML_CODE"], 'selected="selected"') === false){
				$arResult["QUESTIONS"][$key]["HTML_CODE"] = str_replace(">".$arrAnswersVarname[$key][0]["ANSWER_TEXT"]."</", ' selected="selected">'.$arrAnswersVarname[$key][0]["ANSWER_TEXT"]."</", $arResult["QUESTIONS"][$key]["HTML_CODE"]);
			}
			if($arResult["QUESTIONS"][$key]["STRUCTURE"][0]["FIELD_TYPE"] == textarea){
				$arResult["QUESTIONS"][$key]["HTML_CODE"] = str_replace("></textarea>", '>'.$arrAnswersVarname[$key][0]["USER_TEXT"]."</textarea>", $arResult["QUESTIONS"][$key]["HTML_CODE"]);
			}
		}
