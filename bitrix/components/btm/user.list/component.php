...
if (strLen($arParams["USER"]) <= 0) {
    $arResult["ERROR_MESSAGE"] = "Íå ââåäåíû äàííûå ïî Ïîëüçîâàòåëÿì!<br />";
}

if (strLen($arParams["FORM_ID"]) <= 0) {
    $arResult["ERROR_MESSAGE"] = "Íå ââåäåíû äàííûå ïî Ðåçóëüòàòàì ïîëüçîâàòåëåé!<br />";
}
/* --------------------------------------------------- */
//          ÔÎÐÌÈÐÓÅÌ ÔÈËÜÒÐ ÏÎ ÀËÔÀÂÈÒÓ             //
/* --------------------------------------------------- */
...
if ($arResult["ERROR_MESSAGE"] == '') {
    ...
    $rsUsers = CUser::GetList(($by = "id"), ($order = "asc"), $filter, array("SELECT" => array("UF_*"))); // âûáèðàåì ïîëüçîâàòåëåé
    $countUsers = 0;
    $resultFormId = "";
    ...
    //ÑÎÐÒÈÐÎÂÊÀ ÐÅÇÓËÜÒÈÐÓÞÙÅÃÎ ÌÀÑÑÈÂÀ
    for ($k = 0; $k < $countCategory; $k++) {
