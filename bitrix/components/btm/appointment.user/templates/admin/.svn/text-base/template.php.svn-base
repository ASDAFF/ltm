	...
	<form action="<?=$arResult["LINK"]?>" method="post" name="appointment">
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="7" class="regist_info">
        ...
        <tr>
          ...
          <td><?=$arResult["RECIVER"]["NAME"]?> Company: <?=$arResult["RECIVER"]["COMPANY"]?><input name="id" type="hidden" value="<?=$arResult["SENDER"]["ID"]?>" /></td>
        </tr>
        <tr class="chet">
      ...
      <div align="right"><input name="to" type="hidden" value="<?=$arResult["RECIVER"]["ID"]?>" /><input name="form" type="hidden" value="send" /><input name="submit" type="submit" value="Submit" class="send_reg" /></div>
    </form>
	<?
	}
}
