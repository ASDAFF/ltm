...
if ($_SERVER['REQUEST_METHOD']=="POST" && !empty($action))
{
	...
	if (empty($arError))
	{
		if ($action == "save")
		{
			$arResult["MESSAGE_SUCCESS"] = GetMessage("PM_MESSAGE_SUCCESS");
			$arResult["SUBJ_SUCCESS"] = $_REQUEST["POST_SUBJ"];
...
if($USER->IsAdmin()){
	$arResult["USERS"]["FROM"] = GetUserName(1);
}
else{
	$arResult["USERS"]["FROM"] = GetUserName($USER->GetID());
}
$arResult["USERS"]["TO"] = GetUserName($arParams["UID"]);
$arResult["USERS"]["TO_LIST"]["LIST"] = array();
$arResult["USERS"]["TO_LIST"]["COUNT"] = 0;
if(isset($arParams["GROUP_WRITE"]) && $arParams["GROUP_WRITE"] != ''){
...
$thisMessage = array();
$thisMessage["POST_SUBJ"] = '';
if(isset($_REQUEST["mes"])){
	$arMessage = CForumPrivateMessage::GetById($_REQUEST["mes"]);
	$thisMessage = $arMessage->Fetch();
	$arrVars = array( "IS_READ" => "Y" );
	CForumPrivateMessage::Update($_REQUEST["mes"], $arrVars);
	$thisMessage["POST_SUBJ"] = "RE: ".$thisMessage["POST_SUBJ"];
}
$arResult["MESS"]["SUBJ"] = $thisMessage["POST_SUBJ"];
