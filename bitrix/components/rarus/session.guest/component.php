...
if($this->StartResultCache(false, array_merge($arParams, $arResult)))
{
	...
	if($oExhib = $rsExhib->GetNextElement(true, false))
	{
		...
		while($arUser = $rsUsers->Fetch())
		{
			...
			$arResultId[] = $arUser[$propertyName];//дописываем id результата заполнени€ формы
		...
		//получение результатов заполнени€ формы регистрации€ дл€ пользователей
		...
		foreach($arTypes as $type)
		{
			...
			//проход по пользовател€м в типе
			foreach ($arResult["SESSION"][$type]["USERS"] as &$arUser)
			{
				//слоты пользователей

				$times =  $req_obj->getSortedFreeTimesAppoint($arUser["ID"]);//свободные таймслоты пользовател€
				...
				$arDescript = str_replace('вАҐ', '', $arDescript);
				//√ород
				$arTown = array();
				...
				/*
				//рарзделение по приоритетным направлени€м и по городам в них
				...
				*/
				...
				//рарзделение по городам в приоритетных направлени€х
				foreach ($arPriorArea as $id=>$area)
				{
					foreach ($area["ITEMS"] as $answerID => $name)
					{
