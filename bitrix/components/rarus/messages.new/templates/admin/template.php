...
if(isset($arResult["MESSAGE_SUCCESS"]) && $arResult["MESSAGE_SUCCESS"] != ''){
    ...
    <form name="REPLIER" id="REPLIER" action="<?=POST_FORM_ACTION_URI?>" method="POST" class="forum-form">
		...
		?>	<input type="text" placeholder="<?= GetMessage("HLM_TO")?>" value="<?=$arResult["USERS"]["TO"]["NAME"]?>" readonly="readonly" style="width: 100%">
			<input name="RECIPIENT" type="hidden" value="<?=$arResult["USERS"]["TO"]["ID"]?>">
		<?
		}
		?>
            <input name="POST_SUBJECT" type="text" placeholder="<?= GetMessage("HLM_SUBJECT_PH")?>" value="<? if($arResult["POST_VALUES"]["POST_SUBJECT"]){ echo $arResult["POST_VALUES"]["POST_SUBJECT"];}else{ echo $arResult["MESS"]["SUBJECT"];}?>" style="width: 100%">
            <textarea style="width: 100%" name="POST_MESSAGE" placeholder="<?= GetMessage("HLM_MESSAGE_PH")?>" class="post_message" cols="55" rows="14"><?=$arResult["POST_VALUES"]["POST_MESSAGE"]?></textarea>
