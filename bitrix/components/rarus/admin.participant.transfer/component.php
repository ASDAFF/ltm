...
if(intval($arParams["USER_ID"]) <= 0){
    $arResult["ERROR_MESSAGE"] = "Ќе введены данные по ѕользователю!<br />";
}
...
if($arResult["ERROR_MESSAGE"] == '')
{
    ...
    if($this->StartResultCache())
    {
    	...
    	while($obElement = $rsElement->GetNextElement(true,false))
    	{
        	...
        	else //если не находитс€ ни в каких группах
        	{
        	    $arItem["STATUS"] = "NONE";
        	}
	...
	if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["save"]) && check_bitrix_sessid())
	{
		foreach ($arResult["ITEMS"] as &$arItem)
		{
			...
			{
				if(isset($_POST["EXH"][$arItem["ID"]]) && "on" == $_POST["EXH"][$arItem["ID"]])//был в неподтвержденных и осталс€
				{
					continue;
				}
