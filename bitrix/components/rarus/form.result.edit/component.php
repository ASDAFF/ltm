				...
				if ($arResult["F_RIGHT"]>=20 || ($arResult["F_RIGHT"]>=15 && $USER->GetID()==$arResultData["USER_ID"]))
				{
					...
					else
					{
						...
						if ($arResult["arResultData"])
						{
							...
							$oldArValues = $arResult["arrVALUES"];
						}
						else
						{
							$arResult["ERROR"] = "FORM_RECORD_NOT_FOUND";
	...
	if (strlen($arResult["ERROR"]) <= 0)
	{
			...
			)
			{
				...
				if (check_bitrix_sessid())
				{
					...
					if(isset($_REQUEST["type"]) && $_REQUEST["type"] == "C")
					{
                         if(isset($_REQUEST["form_text_30"]) && $_REQUEST["form_text_30"] != ''){//company name
    						...
    						if($_REQUEST["form_text_31"] <> $oldArValues["form_text_31"]) //если меняем логин
    						{
    							...
    							$userRs = $userTmp->GetByLogin($newLogin);
    							$userIsset = $userRs->Fetch();
    							...
    							{
    							    $arTmpUsFields["LOGIN"] = $newLogin;//login
    							}
    						}
    						...
    						if($photoSectionID && CModule::IncludeModule("iblock"))
    						{
    							...
    							$success = $bs->Update($photoSectionID, array("NAME" => $arTmpUsFields["WORK_COMPANY"]));
    							if(!$success)
    							{
    								$bs .= $userTmp->LAST_ERROR;
    							}
    						}
    					}
					...
					elseif(isset($_REQUEST["type"]) && $_REQUEST["type"] == "M")
					{
						...
						$emailAnswerID = CFormMatrix::getAnswerRelBase(89, $arParams["WEB_FORM_ID"]);//login
						$nameAnswerdID = CFormMatrix::getAnswerRelBase(84, $arParams["WEB_FORM_ID"]);//Participant first name
						$lastNameAnswerdID = CFormMatrix::getAnswerRelBase(85, $arParams["WEB_FORM_ID"]);//Participant last name

						$arTmpUsFields = Array(
						  "EMAIL"             => strtolower($_REQUEST["form_text_". $emailAnswerID]),
						  "NAME"	          => $_REQUEST["form_text_" . $nameAnswerdID],
						  "LAST_NAME"         => $_REQUEST["form_text_" . $lastNameAnswerdID],
						  "UF_FIO"         => $_REQUEST["form_text_" . $nameAnswerdID]." ".$_REQUEST["form_text_" . $lastNameAnswerdID],
						);
					...
					elseif(isset($_REQUEST["type"]) && $_REQUEST["type"] == "G")//если форма гостя
					{
					    ...
					    */
					    ...
					    $arTmpUsFields = Array(
					        "EMAIL"             => strtolower($_REQUEST["form_text_220"]),
					        "NAME"	          => $_REQUEST["form_text_216"],
					        "LAST_NAME"         => $_REQUEST["form_text_217"],
					        "UF_FIO"         => $_REQUEST["form_text_216"]." ".$_REQUEST["form_text_217"],
					        "WORK_COMPANY" => $_REQUEST["form_text_204"]
					    );
					...
					/*echo "<pre>";print_r($arTmpUsFields);echo "</pre>";
					print_r($strError);
					die();*/
					if ($strError == '' && CFormResult::Update($arParams["RESULT_ID"], $arResult["arrVALUES"], $arParams["EDIT_ADDITIONAL"]))
					{
		...
		$arResult = array_merge(
			...
			array(
				...
				"FORM_TITLE"			=> trim(htmlspecialcharsEx($arResult["arForm"]["NAME"])), // form title

				"FORM_DESCRIPTION" => // form description
					$arResult["arForm"]["DESCRIPTION_TYPE"] == "html" ?
					trim($arResult["arForm"]["DESCRIPTION"]) :
					nl2br(htmlspecialcharsEx(trim($arResult["arForm"]["DESCRIPTION"]))),
		...
		foreach ($arResult["arQuestions"] as $key => $arQuestion)
		{
			...
			$arResult["QUESTIONS"][$FIELD_SID] = array(
				"CAPTION" => // field caption
					...
					nl2br(htmlspecialcharsEx($arResult["arQuestions"][$FIELD_SID]["TITLE"])),
				...
				"TYPE" => $arResult["arQuestions"][$FIELD_SID]["FIELD_TYPE"],
			);
				...
				foreach ($arResult["arAnswers"][$FIELD_SID] as $key => $arAnswer)
				{
					...
					switch ($arAnswer["FIELD_TYPE"])
					{
						case "radio":
							...
							break;
						case "checkbox":
							$arResult["QUESTIONS"][$FIELD_SID]["TYPE"] = "check";
							if (strpos($arAnswer["FIELD_PARAM"], "id=") === false)
							{
								$ans_id = $arAnswer["ID"];
								$arAnswer["FIELD_PARAM"] .= " id=\"".$ans_id."\"";
						...
						case "file":
							...
							if ($arFile = CFormResult::GetFileByAnswerID($arParams["RESULT_ID"], $arAnswer["ID"]))
							{
								if (intval($arFile["USER_FILE_ID"])>0)
								{
									$res .= "<a title=\"".GetMessage("FORM_VIEW_FILE")."\" target=\"_blank\" class=\"tablebodylink\" href=\"/bitrix/tools/form_show_file.php?rid=".$arParams["RESULT_ID"]."&hash=".$arFile["USER_FILE_HASH"]."&lang=".LANGUAGE_ID."\">".htmlspecialcharsEx($arFile["USER_FILE_NAME"])."</a>&nbsp;(";
									$a = array("b", "Kb", "Mb", "Gb");
									$pos = 0;
...
/*		$rsUser = CUser::GetByID($arParams["RESULT_USER"]);
		...
		$arResult["QUESTIONS"]["USER_TABLE"]["CAPTION"] = "Table";
		$arResult["QUESTIONS"]["USER_TABLE"]["REQUIRED"] = "N";
		$arResult["QUESTIONS"]["USER_TABLE"]["IS_INPUT_CAPTION_IMAGE"] = "N";
		$arResult["QUESTIONS"]["USER_TABLE"]["HTML_CODE"] = '<input class="inputtext" type="text" size="0" value="'.$thisUser.'" name="form_user_table"/>';
