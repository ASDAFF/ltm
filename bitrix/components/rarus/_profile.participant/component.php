...
$arUserGroups = CUser::GetUserGroup($userId); //группы пользовател€
...
if($this->StartResultCache(false, $lang))
{
	...
	while($obElement = $rsElement->GetNextElement())
	{
    	...
    	//ѕроверка на доступ пользовател€ к этой выставке
    	
    	$confirmedGroupID = $arItem["PROPERTIES"]["USER_GROUP_ID"]["VALUE"];
    	
    	//если пользователь не находитс€ в группе подтвержденных
    	...
    	// id результата заполнени€ формы компании
    	$companyResultID = $arUser["UF_ID_COMP"];
    	
    	//получение ид свойства пользовател€ в котором хранитс€ результат заполнени€ формы участника
    	$userExhibPropertyID = CFormMatrix::getPropertyIDByExh($arItem["ID"]);
    	
    	//id результата заполнени€ формы пользовател€ на текущую выставку
    	$userResultID = $arUser[$userExhibPropertyID];
    	
    	//id формы пользовател€ на текущую выставку
