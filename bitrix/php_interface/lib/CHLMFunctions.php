...
class CHLMFunctions
{
    ...
    function GetEXhibByID($id)
    {
        ...
        $rsElement = CIBlockElement::GetList(array(), array("ID"=>$id), false, array("nTopCount"=>1), array("ID","IBLOCK_ID", "CODE", "NAME", "PROPERTY_USER_GROUP_ID", "PROPERTY_UC_PARTICIPANTS_GROUP", "PROPERTY_C_GUESTS_GROUP", "PROPERTY_UC_GUESTS_GROUP"));
        if($arElement = $rsElement->Fetch())
        {
            return $arElement;
        }
