$(function() {
	...
	$('.reg_buy #step3 input[type="text"]').each(function(){
		...
		$(this).blur(function(){
			var val = $(this).val();
			if(te != 'Имя коллеги' && te != 'Имя коллеги 1' && te != 'Имя коллеги 2' && te != 'Имя коллеги 3' && te != 'Фамилия коллеги' && te != 'Фамилия коллеги 1' && te != 'Фамилия коллеги 2' && te != 'Фамилия коллеги 3' && te != 'Должность коллеги' && te != 'Должность коллеги 1' && te != 'Должность коллеги 2' && te != 'Должность коллеги 3' && te != 'E-mail коллеги' && te != 'E-mail коллеги 1' && te != 'E-mail коллеги 2' && te != 'E-mail коллеги 3' && te != 'Индекс' && te != 'Город' && te != 'http://' && te != 'Введите логин/гостевое имя'){
				//console.log(te);
				if(val == '' && !$(this).next().hasClass('ref_er2')){
					$(this).css({'border':'1px solid #ff1111'}).after('<span class = "not_valid"></span>').after('<p class = "ref_er2">Не заполнено поле "'+te+'".</p>');
				}
				...
				if(te == 'Введите E-mail ещё раз' && !$(this).next().hasClass('ref_er2')){
					var email = $('input[name="form_text_220"]').val();
					if(val != email && !rus.test(val) && mail.test(val)){
						if($('.not_valid57').size() == 0){
							$(this).css({'border':'1px solid #ff1111'}).after('<span class = "not_valid57"></span>').after('<p class = "ref_er57">E-mail не совпадают.</p>');
							$(this).prev().css({'border':'1px solid #ff1111'});
						}
						$('.ajax_yes57').remove();
					}else{
						$(this).css({'border':'none'});
						$(this).prev().css({'border':'none'});
						$('.not_valid57, .ref_er57').remove();
						$(this).after('<span class = "ajax_yes57"></span>');
					}
				}
				if(te == 'Введите пароль'){
					var pas = $('input[placeholder="Повторите пароль"]').val();
					...
					if(val != '' && !rus.test(val) && !$(this).next().hasClass('ref_er2')){
						$('#ex_login').load('/ajax/ex_login.php', {login: val}, function(response, status, xhr){
							if(response > 0){
								...
								if(val != '' && th.next().hasClass('not_valid') && !th.next().hasClass('ajax_yes')){
									th.next().remove();
								}
								if(!th.next().hasClass('not_valid')){
									th.after('<span class = "ajax_yes"></span>');
			...
			if(te == 'Индекс'){
				...
				var number = /^[а-яА-ЯёЁa-zA-Z0-9]+$/;
				if(val != '' && !number.test(val)){
					if($(this).next().next().hasClass('ref_er2')){
						$(this).next().next().remove();
						$('.not_valid2').remove();
					}
					$(this).css({'border':'1px solid #ff1111'}).next().after('<span class = "not_valid2"></span>').after('<p class = "ref_er2">Номер должен состоять только из цифр или букв.</p>');
				}
				if(val != '' && number.test(val)){
					if($(this).next().next().hasClass('ref_er2')){
						$(this).next().next().remove();
			...
			if(te == 'Введите логин/гостевое имя'){
				...
				if(val.length < 3){
					...
					$('.ref_er20').remove();
					$('.ajax_yes30').remove();
					$(this).css({'border':'1px solid #ff1111'}).after('<span class = "not_valid30"></span>').after('<p class = "ref_er20">Логин  обязателен и должен быть не менее 3 символов.</p>');
				}else{
					if($(this).next().hasClass('ref_er20')){
						$(this).css({'border':'none'});
						$('.not_valid30').remove();
						$('.ref_er20').remove();
						$(this).after('<span class="ajax_yes30"></span>');
					}
				}
				if(val != ''){
					var th = $(this);
					$('#ex_login').load('/ajax/ex_login.php', {login: val}, function(response, status, xhr){
						...
						if(response > 0){
							th.css({'border':'1px solid #ff1111'}).after('<span class = "not_valid22"></span>').after('<p class = "ref_er22">Данный логин уже занят.</p>');
							$('.ajax_yes30').remove();
							$('.ajax_yes').remove();
							er_login++;
						}else{
							...
							//if(rus.test(th.val())){
								...
								//th.next().remove();
								th.css({'border':'none'});
								$('.not_valid22, .ref_er22, .not_valid30, .ref_er20').remove();
								th.after('<span class = "ajax_yes"></span>');
							//}
						}
					});
	...
	$('#center').on('click', '.reg_exh .ex_but', function(){
		if($(this).hasClass('but_av')){
			...
			if(ex_count == 0){
				...
				if($('.ref_er_m').size()==0){
					$('.reg_exh #t_step2').after("<p class = 'ref_er_m'>Please select at least one event.</p>");
				}
				errors = true;
			}
			var er_c = 0;
			...
			console.log(errors);*/
			var sel_mer = 0;
			if($('.reg_exh #t_step2 .active2').size() == 0){
				sel_mer = 0;
				if($('.ref_er_m2').size() == 0){
					$('.ex_but').before('<span class = "ref_er_m2">Please select at least one event.</span>');
				}
			}else{
				sel_mer = 1;
				$('.ref_er_m2').remove();
			}
			if(!errors && (er_c == 0 && er_c2 == 0 && areas > 0 && agree > 0 && $('.ref_er2').size() == 0 && sol_u == 0 && sel_mer > 0)){
				//console.log('yes');
		...
		if($(this).hasClass('but_av')){
			...
			$('.reg_buy #step3 input[type="text"]').each(function(){
				...
				if($(this).parent().hasClass('y_ex') || $(this).parent().hasClass('com_b')){
					if(te2 != 'Имя коллеги' && te2 != 'Имя коллеги 1' && te2 != 'Имя коллеги 2' && te2 != 'Имя коллеги 3' && te2 != 'Фамилия коллеги' && te2 != 'Фамилия коллеги 1' && te2 != 'Фамилия коллеги 2' && te2 != 'Фамилия коллеги 3' && te2 != 'Должность коллеги' && te2 != 'Должность коллеги 1' && te2 != 'Должность коллеги 2' && te2 != 'Должность коллеги 3' && te2 != 'E-mail коллеги' && te2 != 'E-mail коллеги 1' && te2 != 'E-mail коллеги 2' && te2 != 'E-mail коллеги 3' && te2 != 'Индекс' && te2 != 'Город' && te2 != 'Страна (other)' && te2 != 'http://' && te2 != 'Введите логин/гостевое имя'){
						var te2 = $(this).attr('placeholder');
						var val = $(this).val();
						if(val == '' && !$(this).next().hasClass('ref_er2')){
			...
			if($('#morning').hasClass('y_ex') && !$('#both_time').hasClass('y_ex')){
				...
				//console.log(vid_u);
				//console.log(count_pr_n);
				if(er_all != 0 || count_pr_n == 0 || vid_u == 0 || ref_er22 > 0){
					$('.reg_buy #t_step3').before('<span class = "ref_er2" style = "margin-top: 10px;display: block;">Внимание! Некоторые поля не заполнены или заполнены некорректно!</span>');
				}else{
					if($('.reg_buy #t_step3').prev().hasClass('ref_er2')){
						$('.reg_buy #t_step3').prev().remove();
			...
			if($('#morning').hasClass('y_ex') && $('#both_time').hasClass('y_ex')){
				...
				console.log(er_all);
				console.log(vid_u);*/
				if(er_all != 0 || vid_u == 0 || count_pr_n == 0 || ref_er22 > 0){
					$('.reg_buy #t_step3').before('<span class = "ref_er2" style = "margin-top: 10px;display: block;">Внимание! Некоторые поля не заполнены или заполнены некорректно!</span>');
				}else{
					if($('.reg_buy #t_step3').prev().hasClass('ref_er2')){
						$('.reg_buy #t_step3').prev().remove();
			...
			//console.log('no');
			//console.log(er_login);
			if(!errors && (er_all == 0 && areas > 0 && agree > 0 && $('.ref_er2').size() == 0 && errors_new > 0 && ref_er22 == 0)){
