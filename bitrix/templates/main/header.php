...
<body>
	...
	<header>
		<div id = "top">
			<?$APPLICATION->IncludeComponent("bitrix:main.include", "", array("AREA_FILE_SHOW" => "file", "PATH" => SITE_TEMPLATE_PATH."/include/logo.php"), false);?>
			<?$APPLICATION->IncludeComponent("bitrix:main.include", "", array("AREA_FILE_SHOW" => "file", "PATH" => SITE_TEMPLATE_PATH."/include/title.php"), false);?>
			<div id = "lang">
			    <span class = "change"><?= GetMessage("CHOOSE_LANGUAGE")?></span>
                <a class = "en<?= (LANGUAGE_ID == "en")?" act":"";?>" href="<?= $APPLICATION->GetCurPageParam("lang_ui=en",array("lang_ui", "EXHIBIT_CODE"));?>"><?= (LANGUAGE_ID == "en")?">":"";?>English</a>
                <a class = "ru<?= (LANGUAGE_ID == "ru")?" act":"";?>" href="<?= $APPLICATION->GetCurPageParam("lang_ui=ru",array("lang_ui", "EXHIBIT_CODE"));?>"><?= (LANGUAGE_ID == "ru")?">":"";?>Русский</a>
				<?//$APPLICATION->IncludeComponent("bitrix:main.include", "", array("AREA_FILE_SHOW" => "file", "PATH" => SITE_TEMPLATE_PATH."/include/lang.php"), false);?>
			</div>
		</div>
	</header>
			    	...
			    	elseif("GUEST" == $userType/* && ("Y" == GUEST_CABINET || $USER->IsAdmin())*/)
			    	{
			    	    ...
			    	    if($arExhib = $rsExhib->Fetch())
			    	    {
			    	        ...
			    	        if(in_array($exhibGoup, $arUserGroups) || $USER->IsAdmin())//если в группе подтвержденных
			    	        {
        			    	    ...
        			    	    <? $APPLICATION->IncludeComponent("bitrix:menu", "participant_bottom",
        			    	        Array(
        			    	            "ROOT_MENU_TYPE" => "guest.bottom",	// Тип меню для первого уровня
        			    	            "MAX_LEVEL" => "2",	// Уровень вложенности меню
        			    	            "CHILD_MENU_TYPE" => "",	// Тип меню для остальных уровней
        			    	            "USE_EXT" => "Y",	// Подключать файлы с именами вида .тип_меню.menu_ext.php
        			    	            "DELAY" => "N",	// Откладывать выполнение шаблона меню
