...
switch ($arParams["ACT"]) {
    ...
    case "evening":
        $arShowedTableCols["Форма посещения"] = array("UF_MORNING", "UF_EVENING");
        $arShowedTableCols["Имя коллеги (вечер)"] = "COLLEAGUE_UF_NAME";
        $arShowedTableCols["Фамилия коллеги (вечер)"] = "COLLEAGUE_UF_SURNAME";
        ...
        <? foreach ($arShowedTableColsFilter as $filterCode => $filterName): ?>
            <div class="filters__item">
                ...
                <input type="text" name="<?= $filterCode ?>" id="<?= $filterCode ?>"
                       value="<?= $arResult["FILTER"][$filterCode] ?>">
            </div>
        <? endforeach; ?>
        ...
        <table class="list" style="min-width: 100%;">
                ...
                <? foreach ($arShowedTableCols as $key => $val): ?>
                    <th>
                        <? if (isset($arShowedTableColsSort[$key])): ?>
                            ...
                            ?>
                            <a href="?sort=<?= $arShowedTableColsSort[$key] ?>&order=<?= $orderSort ?>"
                               class="sort-title <?= $class ?>"><?= $key ?></a>
                        <? else: ?>
                            <?= $key ?>
                        <? endif; ?>
                    </th>
            ...
            foreach ($arResult["USERS_LIST"] as $arUser): ?>
                <tr class="<? if ($i++ % 2): ?>odd<? else: ?>even<? endif ?>">
                    <td class="text-center"><input type="checkbox" name="SELECTED_USERS[]" value="<?= $arUser["ID"] ?>">
                    </td>
                    <? foreach ($arShowedTableCols as $key => $val): ?>
                        <td>
                        ...
                        <div class="action" id="action_<?= $arUser["ID"] ?>">
                            ...
                            <ul class="ul-popup">
                                ...
                                <? switch ($arParams["ACT"]):
                                    case "off":
                                        ?>
                                        <li><a href="" data-user-id="<?= $arUser["ID"] ?>"
                                               class="confirm-participate-button">Подтвердить&nbsp;участие</a></li>
                                        <li><a href="<?= $edithrefUconf ?>" target="_blank"
                                               onclick="newWind('<?= $edithrefUconf ?>', 500, 600); return false;">Редактировать</a>
                                        </li>
                                        ...
                                        <li><a href="" data-user-id="<?= $arUser["ID"] ?>" class="spam-guest-button">В
                                                спам</a></li>
                                        <? break ?>
                                    <? case "spam": ?>
                                        ...
                                        <li><a href="" data-user-id="<?= $arUser["ID"] ?>" class="spam-guest-button">Восстановить</a>
                                        </li>
                                        <? break ?>
                                    <? case "evening": ?>
                                        <li><a href="" data-user-id="<?= $arUser["ID"] ?>"
                                               class="unconfirm-participate-button">Отправить в Неподтвержденные</a>
                                        </li>
                                        <li><a href="<?= $edithrefConf ?>" target="_blank"
                                               onclick="newWind('<?= $edithrefConf ?>', 500, 600); return false;">Редактировать</a>
                                        </li>
                                        <li><? $href = "/admin/service/pass.php?uid=" . $arUser["ID"] ?>
                                            <a
                                                    href="<?= $href ?>"
                                    ...
                                    case "hostbuy": ?>
                                        ...
                                        <li><a
                                                    ...
                                                    target="_blank"
                                                    onclick="newWind('/admin/service/pdf_shedule_guest.php?id=<?= $arUser["ID"] ?>&app=<?= $arResult["EXHIB"]["PROPERTIES"]["APP_ID"]["VALUE"] ?>&exhib=<?= $arResult['EXHIB']['CODE'] ?>&type=p&mode=pdf', 600, 700); return false;">Генерировать
                                                расписание</a></li>
                                        <li><a
                                                    ...
                                                    target="_blank"
                                                    onclick="newWind('/admin/service/wishlist_guest.php?id=<?= $arUser["ID"] ?>&app=<?= $arResult["EXHIB"]["PROPERTIES"]["APP_ID"]["VALUE"] ?>&exhib=<?= $arResult['EXHIB']['CODE'] ?>&type=p&mode=pdf', 600, 700); return false;">Генерировать
                                                вишлист</a></li>
                                        <? if ($arParams["ACT"] == "hostbuy" && (isset($arResult["EXHIB"]["PROPERTY_APP_HB_ID_VALUE"]) && $arResult["EXHIB"]["PROPERTY_APP_HB_ID_VALUE"] != '')) {
                                            ...
                                            <li><a
                                                        ...
                                                        target="_blank"
                                                        onclick="newWind('/admin/service/pdf_shedule_guest_hb.php?id=<?= $arUser["ID"] ?>&app=<?= $appId ?>&exhib=<?= $arResult['EXHIB']['CODE'] ?>&type=p&mode=pdf', 600, 700); return false;">Генерировать
                                                    расписание HB</a></li>
                                            <li><a
                                                        href="/admin/service/wishlist_guest_hb.php?id=<?= $arUser["ID"] ?>&app=<?= $appId ?>&exhib=<?= $arResult['EXHIB']['CODE'] ?>&type=p&mode=pdf"
                                                        target="_blank"
                                                        onclick="newWind('/admin/service/wishlist_guest_hb.php?id=<?= $arUser["ID"] ?>&app=<?= $appId ?>&exhib=<?= $arResult['EXHIB']['CODE'] ?>&type=p&mode=pdf', 600, 700); return false;">Генерировать
                                                    вишлист HB</a></li>
                                            <?
                                        }
                                        ?>
									...
									<input class="custom-buttom" type="submit" name="cancell-participation" disabled value="Отменить участие">*/ ?>
                                        <? break ?>
                                    <? endswitch ?>
                                <li><a href="javascript:(void);" data-user-id="<?= $arUser["ID"] ?>"
                                       class="in-storage-button">Поместить&nbsp;в&nbsp;хранилище</a></li>
                            </ul>
                        </div>
                    </td>
                </tr>
