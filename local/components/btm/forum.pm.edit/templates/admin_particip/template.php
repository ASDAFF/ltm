...
if(isset($arResult["MESSAGE_SUCCESS"]) && $arResult["MESSAGE_SUCCESS"] != ''){
	...
	?>
	<div class="error"><?=ShowError($arResult["ERROR_MESSAGE"], "error");?></div>
	<?
	endif;?>
    ...
    <form name="REPLIER" id="REPLIER" action="<?=POST_FORM_ACTION_URI?>" method="POST" onsubmit="return ValidateForm(this);"<?
        ...
        <?=bitrix_sessid_post()?>
        <input name="id" type="text" placeholder="To" value="To Administration" readonly="readonly" />
        <input name="POST_SUBJ" id="POST_SUBJ" type="text" placeholder="Subject" value="<? if($arResult["POST_VALUES"]["POST_SUBJ"]){ echo $arResult["POST_VALUES"]["POST_SUBJ"];}else{ echo $arResult["MESS"]["SUBJ"];}?>" tabindex="<?=$tabIndex++;?>" size="70" />
