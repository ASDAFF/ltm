            ...
            <div class="profil-field pull-left">
                <? foreach ($arShowQuestionBlock["ITEMS"] as $arShowQuestion):
                    ...
                    <? if (in_array($FIELD_SID, $arPriorArea)) {
                        continue;
                    }//пропуск приоритетных направлений
                    ?>
                        ...
                        <? foreach ($arPriorArea as $SID): ?>
                            ...
                            <div class="check-priority">
                                <div class="priority-check-all">
                                    <label class="check-all <? if (count($arQuestion["STRUCTURE"]) == count($arValues)): ?>active-all<? endif; ?>"
                                           for="check_<?= $SID ?>_ALL" type="checkbox"></label>
                                    <input id="check_<?= $SID ?>_ALL" type="checkbox" name="<?= $SID ?>_ALL" value=""
                                           class="none"
                                           <? if (count($arQuestion["STRUCTURE"]) == count($arValues)): ?>checked='checked'<? endif; ?>/>

                                    <a href="javascript:void(0);" class="priority-toggle priority-name">
                                        <ins><?= $arQuestion["CAPTION"] ?></ins>
                                    </a>
                                ...
                                <div class="priority-items" id="priority-items-<?= randString(5) ?>">
                                    <? foreach ($arQuestion["STRUCTURE"] as $arAnswerRes): ?>
                                        <label class="check-group <?= (in_array($arAnswerRes["ID"], $arValues)) ? "active-group" : ""; ?>"
                                               for="check_<?= $SID ?>_<?= $arAnswerRes["ID"] ?>"
                                               type="checkbox"><?= $arAnswerRes["MESSAGE"] ?></label>
                                        <input id="check_<?= $SID ?>_<?= $arAnswerRes["ID"] ?>" type="checkbox"
                                               name="form_checkbox_<?= $SID ?>[]" value="<?= $arAnswerRes["ID"] ?>"
                                               class="none" <?= (in_array($arAnswerRes["ID"], $arValues)) ? "checked='checked'" : ""; ?>/>
                                    <? endforeach; ?>
                                </div>
                            </div>
                        <? endforeach; ?>
