...
try{
	...
	if($exhCode && CModule::IncludeModule("iblock"))
	{
	    ...
	    if($arExhib = $rsExhib->Fetch())
	    {
	        ...
	        $formPropName = CFormMatrix::getPropertyIDByExh($arExhib["ID"]);//получение имени свойства пользовател€ дл€ текущей выставки
	...
	//тут запрещаетс€ редактирование
	if("Y" != $exhGuestEdit)
	{
	    echo "<p style='color:red;'>–едактирование закрыто администратором!</p>";
	...
	//сохранение имени, фамилии и фио в пол€ пользовател€
	$fieldNameName = "form_text_216"; //им€
	$fieldLastNameName = "form_text_217";//фамили€
	...
	if(isset($_REQUEST["formresult"]) && $_REQUEST["formresult"] == "editok")
	{
	    ...
	    echo "<p style='color:red;'>¬несенные изменени€ сохранены</p>";
	}
	...
	Array(
		...
		"QUESTION_TO_SHOW" => array(
			array("ITEMS"=>array(
				...
				array("ID"=>"SIMPLE_QUESTION_773", "TITLE"=>"јдрес"),
				...
				array("ID"=>"SIMPLE_QUESTION_166", "TITLE"=>"ќписание компании"),
